...
/*
 * This class implements all the sellenium methods and parameters for initializing the driver used in your test.
 * All methods created in this class must have reuse characteristics
 */

public class CoreQA {

	private static WebDriver driver;
	private static Boolean executePrintSteps = false;
	private static final long DEFAULT_WAIT_TIMEOUT = 10;
	private static final long IMPLICITLY_WAIT_TIMEOUT = 10;

	public void setPreAction(Boolean printStep) {
		executePrintSteps = printStep;
	}

	public void start(String parBrowser) {
		String title;
		// set the path location of firefox driver
		// System.setProperty("webdriver.firefox.bin", "C:location of firefox driver");
		try {
		...
		if (title.equals("ERROR")) {
			...
			case "chrome":
				...
				optionsC.addArguments(
						Arrays.asList("disable-infobars", "ignore-certificate-errors", "start-maximized"));
				driver = new ChromeDriver(optionsC);
				break;
			case "headless":
				ChromeDriverManager.getInstance().version("76.0.3809.68").setup();
				ChromeOptions optionsH = new ChromeOptions();
				optionsH.addArguments(Arrays.asList("disable-infobars", "ignore-certificate-errors",
						"window-size=2560,1440", "headless"));
	...
	public void captureScreen(String nome) {
		...
		File destination = new File(System.getProperty("user.dir") + "\\target\\Screenshots\\" + nome + ".jpg");
		try {
	...
	public List<WebElement> findElemsAll(String parValue, String... parType) {
		List<WebElement> elements;
		final By locator = getLocatorBy(parValue, parType);
		try {
			elements = driver.findElements(locator);
	...
	public void zoomAjdustment(String zoomPercentage) {
		JavascriptExecutor js = (JavascriptExecutor) driver;
		String zoomInJS = "document.body.style.zoom='" + zoomPercentage + "'";
